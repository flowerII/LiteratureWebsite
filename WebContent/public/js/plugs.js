	...
	$.fn.extend({
		Editor: function(height,width,model,length){
			...
			var editor = new wangEditor($text);
			 ...
			 for(var i=0;i<50;i++){
			 	var emotion={
				 	icon: '../../public/fonts/static/emotions/default/'+num+'.gif',
				 	value: '['+num+']' 
				 }
			 	lis.push(emotion);
			 	num++;
		    ...
		    // 普通的自定义菜单
		    if(!model){
		    	editor.config.uploadImgUrl = 'http://localhost:8080/LiteratureWebsite/upload';
			 	editor.config.uploadImgFileName = 'myImg'
		    	editor.config.menus = [
			        'source',
			        '|',
			        'bold',
